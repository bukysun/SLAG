...
from config import CONSOLE, TWOWIKIMULTIHOPQA_WIKI_JSON_PATH
from eval.twowikimultihopqa_evaluation import exact_match_score, f1_score
...
def load_samples() -> Generator[Sample, None, None]:
    ...
    with open(TWOWIKIMULTIHOPQA_WIKI_JSON_PATH, "r") as f:
        data = json.load(f)
...
if __name__ == "__main__":
    ...
    for func_method, m_name in zip(func_methods, method_names):
        preds = evaluate_2wikimultihopqa(
            func_solution=func_method,
            method_name=m_name,
            top_k=5,
            llm_name="gpt35-1106",
        )
        with open(f"./outputs/2wikimultihopqa/2wikimultihopqa_{m_name}.json", "w") as f:
